...
We are considering, in particular, the problem of archive files, usually .zip files including chemistry data. These are generated by chemists to fulfill the requirement of funding agencies to make the outcome of their research available. These files are typically submitted as supplementary data when publishing scientific articles or stored on repositories such as [Zenodo](zenodo.org), [Figshare](figshare.com), or university repositories such as [Yareta](https://yareta.unige.ch/#/home) for the [University of Geneva](unige.ch), etc.

![image](chemBox.png)

What to do to make these .zip files including chemistry data [FAIR](https://en.wikipedia.org/wiki/FAIR_data)? In other words: How to make them Findable, Accessible, Interoperable and Reusable.

This involves:
- Making recommendations to all statkeholder on the data chain (from chemistry to database providers)
- Make uidelines for the creation of tools to generate fair data, work on 
